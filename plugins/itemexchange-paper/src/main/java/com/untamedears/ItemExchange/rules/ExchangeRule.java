...
public final class ExchangeRule extends ExchangeData {
	...
	public void trace(ItemStack item) {
		...
		addModifiers(BookModifier.fromItem(item), EnchantStorageModifier.fromItem(item), PotionModifier.fromItem(item),
				DamageableModifier.fromItem(item), RepairModifier.fromItem(item));
	}
	...
	public boolean conforms(ItemStack item) {
		...
		if (!Utilities
				.conformsRequiresEnchants(getRequiredEnchants(), meta.getEnchants(), isAllowingUnlistedEnchants())) {
		...
		for (ModifierData modifier : getModifiers()) {
			if (!modifier.conforms(item)) {
				this.plugin
						.debug("[ExchangeRule] [" + modifier.getClass().getSimpleName() + "] Modifier rejected that.");
				return false;
			}
		}
		return true;
		...
		if (ItemExchangePlugin.CAN_ENCHANT.contains(getMaterial())) {
			for (Map.Entry<Enchantment, Integer> requiredEnchant : getRequiredEnchants().entrySet()) {
				if (requiredEnchant.getValue() == ANY) {
					info.add(
							ChatColor.AQUA + EnchantNames.findByEnchantment(requiredEnchant.getKey()).getDisplayName());
				}
				else {
					info.add(
							ChatColor.AQUA + EnchantNames.findByEnchantment(requiredEnchant.getKey()).getDisplayName() +
	...
	public Type getType() {
		...
		return raw == null ? Type.BROKEN :
				"INPUT".equalsIgnoreCase(raw) ? Type.INPUT : "OUTPUT".equalsIgnoreCase(raw) ? Type.OUTPUT : Type.BROKEN;
	}
	...
	public String getListing() {
		...
		if (Strings.isNullOrEmpty(listing)) {
			...
			if (!Strings.isNullOrEmpty(displayName)) {
				niceName += " " + ChatColor.WHITE + ChatColor.ITALIC + "\"" + displayName + ChatColor.WHITE +
						ChatColor.ITALIC + "\"";
			}
			return niceName;
		}
	...
	public ItemStack toItem() {
		ItemStack item = NBTCompound.processItem(ItemExchangePlugin.RULE_ITEM.clone(),
				(nbt) -> nbt.setCompound("ExchangeRule", NBTSerialization.serialize(this)));
		ItemAPI.handleItemMeta(item, (ItemMeta meta) -> {
