...
import com.untamedears.realisticbiomes.breaker.TreeDelegate;
...
public class TreeGrower extends AgeableGrower {
     ...
     * @param mat   Sapling material
     */
    private static Block clearBigTreeSaplings(Block block, Material mat) {
        Block northwest = null;
    ...
    public boolean setStage(Plant plant, int stage) {
        ...
        if (!(block.getBlockData() instanceof Sapling) && block.getType() != Material.RED_MUSHROOM && block.getType() != Material.BROWN_MUSHROOM) {
            return true;
        }
        ...
        Block northwest = null;
        if (canBeBig) {
            northwest = clearBigTreeSaplings(block, mat);
        } else {
            block.setType(Material.AIR);
        }
