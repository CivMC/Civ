...
import com.untamedears.realisticbiomes.noise.CropNoise;
...
public class RBConfigManager extends ConfigParser {
    ...
    protected boolean parseInternal(ConfigurationSection config) {
        ...
        loadSchematics();
        FarmBeaconManager farmBeaconManager = FarmBeaconManager.fromConfiguration(config);
        biomeConfiguration = BiomeConfiguration.fromConfiguration(CropNoise.fromConfiguration(config), farmBeaconManager, config.getMapList("climate"), biomeAliases);
        plantConfigs = parsePlantGrowthConfig(config.getConfigurationSection("plants"), farmBeaconManager, biomeConfiguration);
    ...
    private Set<PlantGrowthConfig> parsePlantGrowthConfig(ConfigurationSection config, FarmBeaconManager farmBeaconManager, BiomeConfiguration biomeConfiguration) {
        ...
        for (String key : config.getKeys(false)) {
            ...
            if (!current.isString("persistent_growth_period")) {
                logger.warning(
                    "persistent_growth_path not specified at " + current.getCurrentPath());
                continue;
            }
            long persistTime = ConfigHelper.parseTime(current.getString("persistent_growth_period"), TimeUnit.MILLISECONDS);
            Climate idealClimate = new Climate(
                current.getDouble("ideal_temperature"),
                current.getDouble("ideal_humidity"),
                current.getBoolean("saline", false),
                current.getBoolean("hell", false));
            ...
            BreakerConfig breaker = BreakerConfig.fromConfig(current.getConfigurationSection("breaker"));
            PlantGrowthConfig growthConfig = new PlantGrowthConfig(name, id, item, greenHouseRates, soilBoniPerLevel,
                maximumSoilLayers, maximumSoilBonus, allowBoneMeal, idealClimate, persistTime, biomeConfiguration, breaker, needsLight, grower, vanillaMats,
                canBePlantedDirectly, needsToBeWaterLogged, farmBeaconManager);
    ...
    private IArtificialGrower parseGrower(ConfigurationSection section, ItemStack item) {
        ...
        switch (section.getString("type").toLowerCase()) {
            ...
            case "column":
                ...
                boolean instaBreakTouching = section.getBoolean("insta_break_touching", false);
