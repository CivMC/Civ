...
public class PhantomOfTheOperaConfig extends SimpleHackConfig {
	...
	protected void wireup(ConfigurationSection config) {
		...
		if (this.timeSinceRestCap == -1) {
			plugin().info("[PhantomOfTheOpera] TIME_SINCE_REST will not be capped.");
		}
		else if (this.timeSinceRestCap < -1) {
			plugin().warning("[PhantomOfTheOpera] TIME_SINCE_REST invalid value, defaulting.");
			this.timeSinceRestCap = 80_000;
		}
		else if (this.timeSinceRestCap < 72_000) {
			plugin().warning("[PhantomOfTheOpera] TIME_SINCE_REST capped below spawning value.");
		}
		else {
			plugin().info("[PhantomOfTheOpera] TIME_SINCE_REST capped to: " + this.timeSinceRestCap);
		}
		// Parse phantom spawning at night
		this.nightSpawn = config.getBoolean("nightSpawn", true);
		if (this.nightSpawn) {
			plugin().info("[PhantomOfTheOpera] Phantoms are now allowed to spawn during the night.");
		}
		else {
			plugin().info("[PhantomOfTheOpera] Phantoms are no longer allowed to spawn during the night.");
		}
		// Parse phantom spawning during storms
		this.stormSpawn = config.getBoolean("stormSpawn", true);
		if (this.stormSpawn) {
			plugin().info("[PhantomOfTheOpera] Phantoms are now allowed to spawn during storms.");
		}
		else {
			plugin().info("[PhantomOfTheOpera] Phantoms are no longer allowed to spawn during storms.");
		}
	}
