...
public class SnitchManager {
	...
	public void cullSnitches() {
		...
		long timeThreshold = System.currentTimeMillis() - (
			86400000L * plugin.getConfigManager().getMaxSnitchLifetime());

		for (Snitch snitch : getAllSnitches()) {
			final Group group = snitch.getGroup();
			if (group == null) {
				plugin.getLogger().log(Level.WARNING, "Snitch at {0} lacks a valid group, skipping cull.",
					snitch.getLoc());
				continue;
			}
			...
			if (performCull == null) {
				...
				for (UUID mod : group.getAllMembers()) {
					playerName = mod;
					if (!NameAPI.getGroupManager().hasAccess(group, playerName,
							PermissionType.getPermission(
							"LIST_SNITCHES"))) { // If they have permission to break blocks.
						continue;
					}
					if (!uuids.containsKey(playerName)) {
						OfflinePlayer player = Bukkit.getOfflinePlayer(playerName);
			...
			for (Snitch snitch: snitchesById.values()) {
				...
				if (reinfGroup == null) {
					plugin.getLogger().log(Level.SEVERE,
						"Juke at {0} is reinforced to a group that no longer exists: {1} (former id: {2}) - removing",
						new Object[] {
							snitch.getLoc(), (snitchGroup != null ? snitchGroup.getName() : null), pRein.getGroupId()
						});
					final String worldName = snitch.getLoc().getWorld().getName();
					final int x = snitch.getX();
					final int y = snitch.getY();
				...
				if (snitchGroup == null || !reinfGroup.getName().equals(snitchGroup.getName())) {
					plugin.getLogger().log(Level.INFO, "Found mismatched juke at {0} correcting from {1} to {2}",
						new Object[] {
							snitch.getLoc(), (snitchGroup != null ? snitchGroup.getName() : null), reinfGroup.getName()
						});
